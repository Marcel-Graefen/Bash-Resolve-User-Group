...
resolve_user_and_group() {
  ...
  if ! user_exists "$SET_USER"; then
    ...
    if ! user_exists "$SET_USER"; then
      ...
      if [[ "$SHOW_WARNING" == "true" ]]; then
        echo "Warning: Neither current USER nor default USER ('$default_user') exist. Clearing SET_USER and SET_UID."
      fi
      SET_USER=""
      SET_UID=""
      ...
      if [[ "$SHOW_WARNING" == "true" ]]; then
        echo "Warning: USER did not exist, switched to default USER '$SET_USER'."
      fi
  ...
  if ! group_exists "$SET_GROUP"; then
    ...
    if group_exists "$default_group"; then
      ...
      if [[ "$show_warning" == "true" ]]; then
        echo "Warning: GROUP did not exist, switched to default GROUP '$default_group'."
      fi
      ...
      if [[ "$show_warning" == "true" ]]; then
        echo "Warning: Neither current GROUP nor default GROUP exist. Clearing GROUP and GID."
      fi
      SET_GROUP=""
      SET_GID=""
